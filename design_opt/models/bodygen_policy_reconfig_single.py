...
class BodyGenPolicyReconfigSingle(Policy):
    def __init__(self, cfg, agent):
        ...
        self.reconfig_action_logits = nn.Linear(cur_dim, self.control_action_dim)
    ...
    def forward(self, x):
        ...
        if len(x_dict['reconfig']) > 0:
            obs, edges, _, num_nodes, num_nodes_cum_reconfig, body_ind, body_depths, body_heights, transformer_obs = self.batch_data(x_dict['reconfig'])
            ...
            reconfig_logits = self.reconfig_action_logits(x)
            reconfig_dist = torch.distributions.Bernoulli(logits=reconfig_logits)
        else:
            num_nodes_cum_reconfig = None
            reconfig_dist = None

        return control_dist, reconfig_dist, node_design_mask, design_mask, total_num_nodes, num_nodes_cum_control, num_nodes_cum_reconfig, x[0][0].device

    def select_action(self, x, mean_action=False):

        control_dist, reconfig_dist, node_design_mask, _, total_num_nodes, _, _, device = self.forward(x)
        ...
        if reconfig_dist is not None:
            reconfig_action = torch.round(reconfig_dist.mean) if mean_action else reconfig_dist.sample()
        else:
        ...
        if reconfig_action is not None:
            action[node_design_mask['reconfig'], self.control_action_dim: 2 * self.control_action_dim] = reconfig_action
        return action
    ...
    def get_log_prob(self, x, action):
        ...
        control_dist, reconfig_dist, node_design_mask, design_mask, total_num_nodes, num_nodes_cum_control, num_nodes_cum_reconfig, device = self.forward(x)
        ...
        # reconfig log prob
        if reconfig_dist is not None:
            reconfig_action = action[node_design_mask['reconfig'], self.control_action_dim: 2 * self.control_action_dim]
            ...
            action_log_prob[design_mask['reconfig']] = reconfig_action_log_prob

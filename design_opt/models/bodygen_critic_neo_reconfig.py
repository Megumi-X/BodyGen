...
class BodyGenNeoReconfigValue(nn.Module):
    def __init__(self, cfg, agent):
        ...
        if not agent.cfg.uni_obs_norm:
            ...
            # self.norm = None
            self.skel_norm = self.attr_norm = self.reconfig_norm = self.control_norm = None
        ...
        if 'mlp' in cfg:
            ...
            self.skel_mlp = self.attr_mlp = self.reconfig_mlp = self.control_mlp = None
    ...
    def forward(self, x):
        ...
        if len(x_dict['reconfig_design']) > 0:
            x, edges, _ , transformer_obs = self.process_input(x_dict['reconfig_design'])
            ...
            value_nodes[node_design_mask['reconfig_design']] = self.reconfig_value_head(x)

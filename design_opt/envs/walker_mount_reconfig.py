...
class WalkerMountReconfigEnv(MujocoEnv, utils.EzPickle):
    def __init__(self, cfg, agent):
        self.cur_t = 0
        self.cfg = cfg
        self.agent = agent
        if self.cfg.xml_name == "default":
            self.model_xml_file = os.path.join(cfg.project_path, "assets", "mujoco_envs", "walker_mount.xml")
        else:
    ...
    def step(self, a):
        ...
        # reconfig stage
            ...
            ob = self._get_obs()
            reward = -np.sum(self.actuator_masks) * self.cfg.reconfig_specs.get('reconfig_reward_scale', 0.)
            termination = truncation = False
